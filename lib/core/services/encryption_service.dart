...
/// Cryptographic helpers used for local authentication and secret handling.
class EncryptionService {
  ...
  Uint8List decryptFile(EncryptedData encrypted, Uint8List key) {
    ...
    final ciphertextWithTag =
        Uint8List(encrypted.ciphertext.length + encrypted.tag.length);
    ciphertextWithTag.setRange(
        0, encrypted.ciphertext.length, encrypted.ciphertext);
    ciphertextWithTag.setRange(
        encrypted.ciphertext.length, ciphertextWithTag.length, encrypted.tag);
  ...
  Future<String> hashPassword(String password, Uint8List salt) async {
    final hash = await compute(
        _hashPasswordIsolate, {'password': password, 'salt': salt});
    // Store as: iterations:salt:hash (base64 encoded)
    final encoded = base64Encode(
        utf8.encode('100000:${base64Encode(salt)}:${base64Encode(hash)}'));
    return encoded;
  }
...
class EncryptedData {
  ...
  factory EncryptedData.fromBinary(Uint8List data) {
    ...
    // Read ciphertext
    final ciphertextLen = (data[offset] << 24) |
        (data[offset + 1] << 16) |
        (data[offset + 2] << 8) |
        data[offset + 3];
    offset += 4;
