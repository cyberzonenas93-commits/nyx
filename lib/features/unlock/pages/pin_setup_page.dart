...
class PinSetupPage extends StatefulWidget {
  ...
  final bool
      isResettingPIN; // True if resetting an existing secondary vault PIN
  final String?
      vaultIdToReset; // The ID of the secondary vault whose PIN is being reset
...
class _PinSetupPageState extends State<PinSetupPage> {
  ...
  Widget build(BuildContext context) {
    return PopScope(
      canPop: widget
          .isChangeMethod, // Allow back navigation only if changing method
                        ...
                        Row(
                          ...
                          children: List.generate(6, (index) {
                            final isFilled = index <
                                (_isConfirming ? _confirmPin : _pin).length;
                            return Container(
  ...
  Future<void> _completeSetup() async {
    ...
    if (_pin.startsWith('0')) {
      setState(() {
        _errorMessage =
            'PIN cannot start with 0. Please enter a PIN starting with 1-9.';
        _pin = '';
        _confirmPin = '';
        _isConfirming = false;
      });
    ...
    try {
      ...
      if (widget.isSecondaryVault &&
          widget.vaultName != null &&
          widget.vaultTriggerCode != null) {
        ...
        try {
          if (widget.isResettingPIN && widget.vaultIdToReset != null) {
            ...
            await multiVaultService.updateSecondaryVaultPIN(
                widget.vaultIdToReset!, pinHash, saltHex);

            debugPrint(
                '[PinSetupPage] Secondary vault PIN reset for vault: ${widget.vaultIdToReset}');
            ...
            if (mounted) {
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  content:
                      Text('PIN reset successfully for "${widget.vaultName}"!'),
            ...
            if (mounted) {
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  content:
                      Text('Vault "${widget.vaultName}" created successfully!'),
                  backgroundColor: AppTheme.accent,
                ),
              );
              ...
              content: Column(
                ...
                children: [
                  ...
                  const Text(
                    ...
                    style: TextStyle(
                        color: AppTheme.text, fontSize: 15, height: 1.5),
                  ),
                  const SizedBox(height: 16),
                  Text(
      ...
      if (mounted) {
        setState(() {
          _isLoading = false;
          _errorMessage =
              'Failed to set up PIN: $e. Please check console for details.';
          _pin = '';
          _confirmPin = '';
          _isConfirming = false;
        });
