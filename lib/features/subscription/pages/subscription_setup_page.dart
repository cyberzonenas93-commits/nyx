...
class _SubscriptionSetupPageState extends State<SubscriptionSetupPage> {
  ...
  Future<void> _loadProducts() async {
    ...
    if (mounted) {
      setState(() {
        if (_selectedTier == null && subscriptionService.products.isNotEmpty) {
          _selectedTier = subscriptionService.products.any(
            (product) => product.id == SubscriptionTier.yearly.productId,
          )
  ...
  Future<void> _startFreeTrial() async {
    ...
    switch (result) {
      ...
      case PurchaseResult.pending:
        setState(() {
          _errorMessage =
              'Your purchase is pending store confirmation. Once it clears, premium access will unlock automatically.';
        });
        break;
      ...
      case PurchaseResult.unavailable:
        setState(() {
          _errorMessage =
              'Subscriptions are unavailable right now. You can continue with the free tier.';
        });
        break;
      case PurchaseResult.failed:
        setState(() {
          _errorMessage = 'Failed to complete the purchase. Please try again.';
        });
        break;
    }
  }
  ...
  Future<void> _skipSubscription() async {
    try {
      ...
      debugPrint(
          '[SubscriptionSetupPage] Navigation completed, updating auth state...');
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            _FeatureItem(
                              ...
                              description:
                                  'Your vault files stay on your device',
                            ),
                            const SizedBox(height: 16),
                            _FeatureItem(
                          ...
                          children: [
                            ...
                            _SubscriptionCard(
                              ...
                              product: products.firstWhere(
                                (p) =>
                                    p.id == SubscriptionTier.monthly.productId,
                              ...
                              children: [
                                _SubscriptionCard(
                                  ...
                                  product: products.firstWhere(
                                    (p) =>
                                        p.id ==
                                        SubscriptionTier.yearly.productId,
              ...
              Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Center(
                      child: TextButton(
                        onPressed: (_isStartingTrial || _isLoading)
                            ? null
                            : () {
                                debugPrint(
                                    '[SubscriptionSetupPage] Continue with Free Tier button pressed');
                                _skipSubscription();
                              },
                        child:
                            const Text('Continue with Free Tier (5 items max)'),
                      ),
                    ),
                    const SizedBox(height: 8),
                    Center(
